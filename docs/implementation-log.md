...
## Resume markdown по проекту

**Статус:** выполнено

**Цель:** подготовить отдельный markdown-файл с резюме-ориентированным описанием проекта, чтобы его можно было быстро адаптировать под CV и дополнительно прогнать через внешний редактор вроде Claude.

**Чеклист выполненного:**

- [x] Изучен текущий стиль `resume.docx`.
- [x] Подготовлен отдельный markdown с кратким описанием проекта для резюме.
- [x] Вынесены актуальный стек и реальные инженерные результаты проекта.
- [x] Добавлены проверяемые метрики и результаты live-прогонов.
- [x] Подготовлены две готовые версии блока для вставки в резюме.
- [x] Отдельно зафиксированы формулировки, которые стоит и не стоит использовать.

**Изменённые файлы:**

- `docs/resume-rag-chatbot-results.md`
- `docs/implementation-log.md`

**Проверка:**

- Проверено, что `resume.docx` содержит проектный блок с кратким описанием, стеком и bullets.
- Проверено, что новый markdown отражает актуальное состояние проекта, а не старую версию с `Ollama` и `RAGAS`.
- Проверено, что в файле есть как полный материал, так и короткие готовые блоки для CV.

**Известные follow-up пункты:**

- При желании можно сделать ещё одну версию того же блока на английском.
- Если захочешь, следующим шагом можно адаптировать этот markdown прямо под конкретную вакансию.
...
**Цель:** привести `readme.md` в соответствие с текущим состоянием проекта, чтобы README отражал уже не базовый RAG-прототип, а актуальный controlled agentic RAG с новым eval и live-проверками.

**Чеклист выполненного:**

- [x] Полностью обновлено описание проекта в `readme.md`.
- [x] Добавлены актуальные возможности agent runtime.
- [x] Обновлены структура проекта, запуск и конфигурация.
- [x] Добавлен раздел про `agent eval harness`.
- [x] Добавлен раздел с последним подтверждённым online-статусом проекта.
- [x] Зафиксированы ограничения и ссылки на внутреннюю документацию.
...
- Проверено, что README соответствует текущему стеку: `Groq API`, `Qdrant`, `Redis`, `PostgreSQL`, `FastAPI`.
- Проверено, что в README отражены `POST /agent/chat`, agent runtime и eval harness.
- Проверено, что README больше не описывает проект как только базовый retrieval-сервис без agent-layer.
- Тесты не запускались, потому что задача затрагивает только документацию.

**Известные follow-up пункты:**

- При желании можно отдельно добавить в README пример полного agent response JSON.
- Если понадобится публичная версия README для работодателей, можно сделать ещё более короткий showcase-вариант.
