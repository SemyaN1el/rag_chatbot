...
- Нет полноценного orchestration-слоя, который исполняет шаги агента поверх реальных tool calls.
- Нет отдельного router/planner-слоя, который выбирает стратегию обработки запроса на runtime.
- Tools, memory, guardrails и validators только намечены и ещё не интегрированы в единый pipeline.
